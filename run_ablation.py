...
def parse_args():
    ...
    parser.add_argument("--seeds", type=int, nargs="+", default=[2, 536, 576, 584], help="List of random seeds")
    parser.add_argument("--alphas", type=float, nargs="+", default=[0.3, 0.7], help="Alpha values for Config B and C")
...
def main():
    ...
    for seed in args.seeds:
        ...
        # This baseline doesn't change with alpha, so we run it once per seed.
        ...
        # Fine-tuned CLIP for Config C
        # Fine-tuning depends on the seed, not alpha, so we do it once per seed.
        print("\n[Config C] Fine-tuning CLIP...")
        ...
        for alpha in args.alphas:
            print(f"\n--- Running at Alpha: {alpha} ---")
            
            # Configuration B: Frozen CLIP + Frozen BLIP-2
            print(f"\n[Config B] Frozen CLIP + Frozen BLIP-2 (Alpha={alpha})")
            config_b_path = os.path.join(args.output_dir, f"idx_B_seed_{seed}_alpha_{alpha}")
            run_command([
                py_exec, "src/indexing.py",
                "--data_dir", args.data_dir,
            ...
            eval_b_csv = os.path.join(args.output_dir, f"eval_B_seed_{seed}_alpha_{alpha}.csv")
            run_command([
                py_exec, "evaluate.py",
                "--data_root", args.data_dir,
                "--index_path", config_b_path,
            ...
            res_b.update({"Config": "B", "Seed": seed, "Alpha": alpha, "Finetuned": False})
            all_results.append(res_b)

            # Configuration C: Fine-tuned CLIP + Frozen BLIP-2
            print(f"\n[Config C] Fine-tuned CLIP + Frozen BLIP-2 (Alpha={alpha})")
            config_c_path = os.path.join(args.output_dir, f"idx_C_seed_{seed}_alpha_{alpha}")
            run_command([
                py_exec, "src/indexing.py",
                "--data_dir", args.data_dir,
            ...
            eval_c_csv = os.path.join(args.output_dir, f"eval_C_seed_{seed}_alpha_{alpha}.csv")
            run_command([
                py_exec, "evaluate.py",
                "--data_root", args.data_dir,
                "--index_path", config_c_path,
            ...
            res_c.update({"Config": "C", "Seed": seed, "Alpha": alpha, "Finetuned": True})
            all_results.append(res_c)

    # Final Aggregation
    summary_df = pd.DataFrame(all_results)
    
    # Calculate Mean and Std grouped by Config and Alpha
    metrics = [c for c in summary_df.columns if "Recall" in c or "NDCG" in c or "mAP" in c]
    summary = summary_df.groupby(["Config", "Alpha"])[metrics].agg(['mean', 'std']).reset_index()
